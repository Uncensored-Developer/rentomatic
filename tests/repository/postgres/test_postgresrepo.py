...
def test_repository_list_without_parameters(db_setup, pg_data, pg_session):
    repo = postgresrepo.PostgresRepo(db_setup['postgres'])
    repo_rooms = repo.list()
    assert set([r.code for r in repo_rooms]) == set([r['code'] for r in pg_data])


def test_repository_list_with_code_equal_filter(db_setup, pg_data, pg_session):
    repo = postgresrepo.PostgresRepo(db_setup['postgres'])
    repo_rooms = repo.list(
...
def test_repository_list_with_price_equal_filter(db_setup, pg_data, pg_session):
    repo = postgresrepo.PostgresRepo(db_setup['postgres'])
...
def test_repository_list_with_price_greater_than_filter(db_setup, pg_data, pg_session):
    repo = postgresrepo.PostgresRepo(db_setup['postgres'])
...
def test_repository_list_with_price_between_filter(db_setup, pg_data, pg_session):
    repo = postgresrepo.PostgresRepo(db_setup['postgres'])
